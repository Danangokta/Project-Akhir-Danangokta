...
class PopularMoviePagedListAdapter(public val context: Context) : PagedListAdapter<Movie, RecyclerView.ViewHolder>(MovieDiffCallback()) {
   ...
   //view yang digunakan untuk menampilkan data pada tampilan
    ...
    //proses menginisiasi viewholder pada recyclerview
    ...
    //tampilan list movie pada halaman pertma
    class MovieItemViewHolder (view: View) : RecyclerView.ViewHolder(view) {

        //proses mengikat data untuk nantinya ditampilkan
        fun bind(movie: Movie?,context: Context) {
            itemView.cv_movie_title.text = movie?.title
            itemView.cv_movie_release_date.text =  movie?.releaseDate

            //pemanggilan data dari url
            val moviePosterURL = POSTER_BASE_URL + movie?.posterPath
            Glide.with(itemView.context)
                .load(moviePosterURL)
                .into(itemView.cv_iv_movie_poster);

            //proses unutk membuat list movie bisa di klik dan masuk ke fragmemnt berikutnya
